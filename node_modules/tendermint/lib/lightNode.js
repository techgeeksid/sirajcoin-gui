...
var LightNode = function (_EventEmitter) {
  ...
  function LightNode(peer, state) {
    ...
    _this.maxAge = opts.maxAge || 30 * 24 * 60 * 60;
  ...
  _createClass(LightNode, [{
    key: 'handleError',
    value: function handleError(promise) {
      var _this2 = this;
    ...
    value: async function initialSync() {
      ...
      var tip = status.latest_block_height;
    ...
    value: async function subscribe() {
      ...
      var syncing = false;
      await this.rpc.subscribe({ query: query }, async function (_ref2) {
        var header = _ref2.header;

        // don't start another recursive sync if we are in the middle of syncing
        if (syncing) return;
        syncing = true;
        await _this3.handleError(_this3.syncTo(header.height));
        syncing = false;
      });
    }
  }, {
    key: 'handleHeader',
    value: async function handleHeader(header, commit) {
